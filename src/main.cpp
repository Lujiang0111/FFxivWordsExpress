...
void ExpressFromFile(std::set<std::string> &setWord, const FileInfoInterface *file_info)
{
    if (file_info)
    {
        for (size_t i = 0; i < file_info->GetChildsSize(); ++i)
        {
            ExpressFromFile(setWord, file_info->GetChild(i));
        }

        if (los::files::kRegular == file_info->GetMode())
        {
            if (strstr(file_info->GetName(), ".csv"))
            {
                printf("%s\n", file_info->GetFullName());
                ExpressFromCsv(setWord, file_info->GetFullName());
            }
        }
    }
}
...
int main(int argc, char* argv[])
{
    ...
    auto file_info = los::files::GetFileInfo(path.c_str(), los::files::kNone);

    std::set<std::string> setWord;
    ExpressFromFile(setWord, file_info.get());
